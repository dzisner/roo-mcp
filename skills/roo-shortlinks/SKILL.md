...
The details that go wrong most often:
...
- **Preview link.** `title` and `description` are required. `image` accepts a base64 data URI when uploading a new image; on later edits, send back the `image_url` returned by the server to keep the existing image. **Roo enforces a minimum image size of ~100 KB** — smaller uploads are rejected with `"Link preview image must be at least 100KB."`. Provide a real preview-sized image, not a placeholder.
...
Plan tier changes limits, not behavior — but the limits shape sensible defaults. Every account's live limits are in `roo_whoami`'s response under `effectiveLimits`.
...
- **Custom domains:** 1 on Hop; up to 10 on Enterprise. `roo.ws` and `roo.bz` are Roo-owned defaults and don't count against this limit — use `roo_list_custom_domains` for the authoritative list of the account's actual custom domains.
...
Compose these from the tools above:

- **Campaign launch link.** One shortlink pointing at the teaser URL, plus `roo_set_scheduled_redirect` with a single waypoint at launch time switching to the live page. Times before the waypoint fall through to the base URL (the teaser); times after go to the live page. If the launch needs to be delayed, PATCH the waypoint's timestamp — no need to touch the base URL.

- **Click-count A/B rotation.** One shortlink, `roo_set_click_count_redirect` with an interleaved list of thresholds pointing at variant A, variant B, A, B — split by count. Use `roo_get_shortlink` to read the current click count and monitor split.

- **Lead-capture link.** Create shortlink pointing at the marketing page, then `roo_set_webhook` with `endpoint` set to a Make/Pipedrive/n8n webhook URL and `add_metadata: true`. Every click will POST click metadata to that endpoint; downstream automation handles the CRM upsert.

- **Seasonal QR (reprint-free).** Create shortlink with `roo_create_shortlink` including both `add_ons.qr` (for the code) and `add_ons.scheduled_redirect` (waypoints for each seasonal destination). The QR image is returned inline in the create response — print it once. To change the destination for the next season, PATCH the scheduled redirect via `roo_set_scheduled_redirect`; the QR stays the same.
